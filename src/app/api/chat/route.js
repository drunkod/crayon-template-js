...
async function handleMockMode(userMessage, threadMessages) {
  ...
  if (!location) {
    ...
    async function* generateStream() {
      // Split into words and stream word by word with spaces
      // This makes it clear to the parser that it's text, not JSON
  ...
  try {
    ...
    async function* generateWeatherStream() {
      // For JSON, stream it all at once after a brief delay
